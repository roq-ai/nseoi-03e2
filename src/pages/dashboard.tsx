...
import { getData } from 'apiSdk/data';
...
import { OwnerListPage as ShowCaseList } from 'pages/owners';

function DashboardPage() {
  const { session } = useSession();

  const dataStat = useSWR<PaginatedInterface<DataInterface>>(
    () => `/data`,
    () => getData({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Data',
      result: dataStat?.data?.totalCount,
      isLoading: dataStat.isLoading,
          ...
          <Stack direction="column" spacing={8} flex={{ base: 1, md: 7 }}>
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                ...
                <NextLink href={`owners`} passHref legacyBehavior>
                  <Button
